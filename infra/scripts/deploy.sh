...
if ! docker info | grep -q "Swarm: active"; then
    ...
    echo "Run './init.sh manager' on manager node first"
    exit 1
fi
...
if [ ! -f infra/docker-swarm-stack.yml ]; then
    echo -e "${RED}Error: infra/docker-swarm-stack.yml not found${NC}"
    exit 1
fi

echo -e "${GREEN}Deploying stack: $STACK_NAME${NC}"
docker stack deploy \
    -c infra/docker-swarm-stack.yml \
    --with-registry-auth \
    "$STACK_NAME"
